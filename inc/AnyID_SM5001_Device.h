...
#define DEVICE_RETURN_BAT_FRAME_LEN         0x20
...
#define DEVICE_GATE_STAT_INFO			36
...
#define Device_GateBatCtr(add, ctrMode, snLen)		do{\
													...
													memcpy(g_sGateOpInfo.slvCmd.params + 1 , pFrame, snLen);\
												}while(0) 	
													
#define Device_GetBatSnLen(p)					({\
													u8 lenth = 0;\
													if(*(p + 24) == DEVICE_STR_HEX_0 && *(p + 25) == DEVICE_STR_HEX_0 && *(p + 26) == DEVICE_STR_HEX_0 && *(p + 27) == DEVICE_STR_HEX_0)\
													{ \
														lenth = BAT_SN_LEN - 4;\
													} \
													else \
...
#define Device_GatePlBat(add, v, snLen)                do{\
													g_sGateOpInfo.add = add - 1;\
													g_sGateOpInfo.state = GATE_OP_STAT_TX;\
													g_sGateOpInfo.mode = GATE_MODE_CMD;\
													...
													memcpy(g_sGateOpInfo.slvCmd.params + 1 , pFrame, snLen + 1);\
												}while(0)
