       ...
       || (batAddr == addr && pFrame[UART_RFRAME_POS_CMD] == GATE_FRAME_CMD_BRWBAT))    //借还电池指令是否区分？
    {
        ...
        switch(pOpInfo->cmd)
        {
            ...
            case GATE_FRAME_CMD_SET_OUTINFO:
              ...
              if(!a_CheckStateBit(g_sDeviceTestInfo.flag, GATE_FLAG_DOOR_TEST))      //门开测试无需上报
              {
...
u16 Device_WaterProceRspFrame(u8 *pFrame, WATER_INFO *pOpInfo, u8 len)
{
    ...
    switch(cmd)
    {
        case WATER_CMD_GET_UID:
            if(paramsLen > 0)
            {
				if(memcmp(pOpInfo->smapleUid, pFrame + 20, WATER_UID_LEN))         //防误触 
                {
            ...
            case DEVICE_CMD_VERSION_UPDATA:
                if(paramsLen == FRAM_VERSION_SIZE + 2)
                {
                    if(add == DEVICE_SM5001_ID)
                    {
						 ...
						 if(!memcmp(pFrame, "SM5001_", 7) && (!memcmp(pFrame + 15, "_1000", 5) || !memcmp(pFrame + 15, "_0100", 5)))     //暂只识别主控仓控更新包
						 {
                ...
                if(paramsLen == 5)
                {
                  if(add == DEVICE_SM5001_ID)
                  {
                         if(*(pFrame + 0) <= SOUND_VOCIE_MAX)
                         {
                            ...
                            g_sSoundInfo.test = SOUND_VOICE_TEST_FLAG;   //参数存储，上电配置， 给予提示
          ...
          case GATE_FRAME_CMD_RTNBAT:
                if(paramsLen == DEVICE_RETURN_BAT_FRAME_LEN)
                {
                    ...
                    if(Device_ChkGate(add))
                    {
						...
						Device_GateBatCtr(add, GATE_FRAME_CMD_RTNBAT, Device_GetBatSnLen(pFrame));     //长度校验
						Gate_TxFrame(&g_sGateOpInfo, tick);
                    }
                    else
                    {
        ...
        case GATE_FRAME_CMD_BRWBAT:
          ...
          case GATE_FRAME_CMD_PLANE_BAT:
                if(paramsLen == DEVICE_RETURN_BAT_FRAME_LEN + 1)
                {
                    ...
                    if(Device_ChkGate(add))
                    {
                         if(g_sGateOpInfo.batOpState != GATE_OP_BAT_STAT_ING)            //借还电池过程中可预约电池？？？
                         {
